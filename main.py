...
def save_rendered_results(args, dir, final_color, mesh):
    ...
    for i in np.arange(0, np.pi * 2, np.pi / 180 * 10):
        img, mask = kal_render.render_single_view(mesh, elev=np.pi / 4, azim=float(i),
                                                  radius=2.5,
                                                  background=torch.tensor([1, 1, 1]).to(device).float(),
                                                  return_mask=True)
    ...
    final_video[0].save('video_final_obj.gif', save_all=True, optimize=False, append_images=final_video[1:], loop=0)
